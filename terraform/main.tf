...
resource "aws_instance" "jenkins_docker" {
  ...
  tags = { Name = "${var.project_name}-jenkins-docker" }
}
...
output "jenkins_url" {
  value = "http://${aws_instance.jenkins_docker.public_ip}:8080"
}

output "app_url" {
  value = "http://${aws_instance.jenkins_docker.public_ip}:3000"
}